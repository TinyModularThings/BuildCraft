...
public class GuiGateInterface extends GuiAdvancedInterface
{
	...
	protected void mouseClicked(int i, int j, int k)
	{
		...
		doSlotClick(position, k);
	}
	
	public void doSlotClick(int position, int k)
	{
		if(position < 0)
		{
			return;
	...
	public void handleMouseInput()
	{
		...
		if(wheel != 0)
		{
			...
			int cornerX = (width - xSize) / 2;
			int cornerY = (height - ySize) / 2;
			int id = getSlotAtLocation(i - cornerX, j - cornerY);
			if(id < 0)
			{
				return;
			}
			int times = wheel < 0 ? -wheel : wheel;
			
			for(int z = 0;z<times;z++)
			{
				doSlotClick(id, wheel > 0 ? 0 : 1);
			}
		}
	}
}
