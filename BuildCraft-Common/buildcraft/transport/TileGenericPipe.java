...
public class TileGenericPipe extends TileEntity implements IPowerReceptor, IFluidHandler, IPipeTile, IOverrideDefaultTriggers, ITileBufferHolder,
		IDropControlInventory, IPipeRenderState, ISyncedTile, ISolidSideTile, IGuiReturnHandler, IItemConduit, IEnergyHandler {
	...
	{
		PowerReceiver rec = getPowerReceiver(direction);
		if(rec != null)
		{
			return (int)rec.receiveEnergy(Type.STORAGE, (amount / 10), direction) * 10;
		}
		return 0;
	}
	...
	public int getEnergyStored(ForgeDirection direction)
	{
		...
		if(rec != null)
		{
			return (int)(rec.getEnergyStored() * 10);
		}
		return 0;
	}
	...
	public int getMaxEnergyStored(ForgeDirection direction)
	{
		...
		if(rec != null)
		{
			return (int)(rec.getMaxEnergyStored() * 10);
		}
		return 0;
	}
	...
	/* IItemConduit */
	@Override
	public ItemStack insertItem(ForgeDirection direction, ItemStack item)
	{
		if(BlockGenericPipe.isValid(pipe) && pipe.transport instanceof PipeTransportItems && isPipeConnected(direction))
		{
			int added = injectItem(item, true, direction);
	...
	@Override
	@Deprecated
	public ItemStack insertItem(ForgeDirection direction, ItemStack item, boolean simulate)
	{
		return item;
	}
	...
	@Override
	@Deprecated
	public ItemStack sendItems(ItemStack item, ForgeDirection direction)
	{
		return item;
	}
	...
	protected boolean canPipeConnect(TileEntity with, ForgeDirection side) {
		...
		if(with instanceof IEnderAttuned)
		{
			IEnderAttuned ender = (IEnderAttuned)with;
			boolean flag = false;
			if(pipe.transport instanceof PipeTransportItems)
			{
				flag = ender.canReceiveItems() || ender.canSendItems();
			}
			else if(pipe.transport instanceof PipeTransportFluids)
			{
				flag = ender.canReceiveFluid() || ender.canSendFluid();
			}
			else if(pipe.transport instanceof PipeTransportPower)
			{
				flag = ender.canReceiveEnergy() || ender.canSendEnergy();
			}
			return flag;
		}
