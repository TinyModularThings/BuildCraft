...
public class PipeTransportItems extends PipeTransport {
	...
	private void handleTileReached(TravelingItem item, TileEntity tile) {
		...
		else if(tile instanceof IEnderAttuned)
		{
			if(CoreProxy.proxy.isSimulating(container.worldObj))
			{
				IEnderAttuned ender = (IEnderAttuned)tile;
				ItemStack result = ender.receiveItem(item.getItemStack());
				item.getItemStack().stackSize-=result.stackSize;
				if (item.getItemStack().stackSize > 0) {
					reverseItem(item);
				}
			}
